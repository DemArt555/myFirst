...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := distance(action) // вызовите здесь необходимую функцию
		speed := swimmingMeanSpeed(lengthcountPool, countcountPool, duration) / kmkmhInMsec // вызовите здесь необходимую функцию
		calories := swimmingSpentCalories(lengthlengthPool, countPool, duration, weight) // вызовите здесь необходимую функцию
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
    // ваш код здесь
    (swimmingMeanSpeed(lengthPool, countPool, duration)/kmhInMsec + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
